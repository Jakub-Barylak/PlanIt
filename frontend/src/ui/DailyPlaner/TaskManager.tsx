...
const TaskManager: React.FC<TaskManagerProps> = ({
	...
	useEffect(() => {
		axios.get("/todo_lists/").then((res: AxiosResponse) => {
			setTasks(res.data);
		});
	}, []);
	...
	const handleToggle = (index: number) => {
		...
		axios
			.patch(`/todo_lists/${tasks[index].id}/`, { done: !newTasks[index].done })
			.then(() => {
	...
	const handleRemoveTask = (index: number) => {
		...
		axios
			.delete(`/todo_lists/${tasks[index].id}/`)
			.then(() => {
	...
	return (
		<div
			className={`h-100vh  flex flex-col items-start gap-4 p-4 bg-${
				isDark ? "bg-dark" : "bg-white"
			} `}
		>
			<div
				className={`h-full  w-0.5 bg-${
					isDark ? "gray-500" : "gray-200"
				} absolute ${
					showTaskManager ? "left-[80vw]" : "invisible opacity-0"
				}  z-2'} bottom-4 top-0 transition-all duration-300`}
			></div>

			<span
				onClick={toggleTaskManager}
				//</div>className={`cursor-pointer inline-block mr-5 ${showTaskManager ? 'ml-[-3vw] mt-[1vh]' : 'ml-[-5vw] mt-[0vh] z-10'}`} >
				className={`mr-5 inline-block cursor-pointer ${
					showTaskManager ? "ml-[-3vw] mt-[1vh]" : "ml-[-5vw] mt-[1vh]"
				}  z-10`}
			>
				<div
    ...
    ${
			isDark
				? "border-darkMode-secondary-text bg-gray-900"
    ...
    flex items-center justify-center rounded border-2 `}
			...
			<h2
				className={`mx-auto mt-[-5vh] text-xl ${
					isDark ? "text-gray-400" : "text-gray-700"
				}  ${showTaskManager ? "text-center" : "text-ml-[3vw] text-center  "} `}
				style={{ width: showTaskManager ? "16vw" : "5vw" }}
			>
				{" "}
				To Do
			</h2>
			...
			<div
				className={`h-0.5 bg-${
					isDark ? "gray-500" : "gray-200"
				} mt-[0vh] w-full ${showTaskManager ? "" : "invisible opacity-0"}`}
			></div>
			...
			{showTaskManager && (
				...
				>
					...
					>
						{tasks.map((task, index) => (
							<li
								...
								className={`bg-myCustomBackground flex items-center justify-between text-${
									isDark ? "white" : "black"
								} rounded-2 mb-0 box-border w-full p-1`}
							>
								<Task
									task={task}
					...
					>
						<AddTask onAdd={handleAddTask} flag={flag} setFlag={setFlag} />
					</div>
				</div>
			)}
		</div>
